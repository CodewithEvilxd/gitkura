...
export interface PixelatedImageTrailProps {
  ...
  smoothing?: number;
  imageSize?: number;
  sideZonesOnly?: boolean;
  sideZoneRatio?: number; // e.g. 0.28 = left 28% and right 28%
}
...
const DEFAULT_IMAGES = [
  "/illustrations/vault-safe.png",
  "/illustrations/delta-sync.png",
  "/illustrations/git-tree.png",
  "/illustrations/star-verified.png",
  "/illustrations/telegram-plane.png",
  "/illustrations/s3-bucket.png",
  "/illustrations/nas-server.png",
  "/illustrations/security-padlock.jpg",
  "/illustrations/coffee-cup.png",
  "/illustrations/retro-terminal.jpg",
  "/illustrations/cloud-dispatch.jpg",
];
...
export function PixelatedImageTrail({
  ...
  useEffect(() => {
    ...
    const createTrailImage = () => {
      ...
      // If sideZonesOnly is active, only spawn in left/right margins (Zones 1 and 2)
      if (sideZonesOnly) {
        const containerWidth = container.clientWidth || window.innerWidth;
        const relativeX = interpolatedPointerPosRef.current.x / containerWidth;
        const isInZone1Or2 =
          relativeX <= sideZoneRatio || relativeX >= 1 - sideZoneRatio;
        if (!isInZone1Or2) {
          return;
        }
      }
      ...
      Object.assign(imageElement.style, {
        ...
        zIndex: "5",
        filter: "drop-shadow(0 10px 18px rgba(23, 54, 93, 0.16))",
  ...
  return (
    <div
      ...
      aria-hidden="true"
      className={cn(
        "absolute inset-0 w-full h-full pointer-events-auto touch-none select-none",
        className
      )}
    />
  );
